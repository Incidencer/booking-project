      ...
      handleSubmit,
       ...
       const mutation = useMutation(apiClient.register, {
        onSuccess: async () => {
          ...
          await queryClient.invalidateQueries('validateToken') // проверка валидации токена, да бы страница сама перезагружалась при регистрации
          navigate('/')
        },
        onError: (error: Error) => { 
...
// form registration, h2 - top text, label+input - one element
  return (
            ...
            <input type="password" className="border rounded w-full py-1 px-2 font-normal"{...register('confirmPassword', {
              validate:(val)=>{
                if(!val) {
                  ...
                  return 'Your password dont confirmed'
                }
              }
            })}></input>
