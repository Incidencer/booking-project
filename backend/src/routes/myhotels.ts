...
router.get('/', verifiyToken, async(req: Request, res: Response)=> {
    try {
        const hotels = await Hotel.find({userId: req.userId});
        res.json(hotels);    
    } catch(error) {
        res.status(500).json({message: 'Error fetch hotels'})
    }
})

router.get('/:id', verifiyToken, async(req: Request, res: Response) => {
    // api/my-hotels/345345345345435
...
router.put('/:hotelId', verifiyToken, upload.array("imageFiles"), async (req: Request, res: Response) => {
    try {
        ...
        if(!hotel) {
            return res.status(404).json({message: 'Not found the hotel'})
        }
